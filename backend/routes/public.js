...
// ── Artists — simple list (booking dropdown) ────────────────
router.get("/artists", async (_req, res, next) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, name FROM artists WHERE active = 1 ORDER BY sort_order, name`
    );
    res.json(rows);
  } catch (err) { next(err); }
});

// ── Artists — rich list (public /allartists page) ───────────
...
router.post("/bookings/:ref/receipt", async (req, res, next) => {
  try {
    ...
    await pool.query(
      `INSERT INTO receipts (booking_id, file_url, status) VALUES ($1,$2,'pending')`,
      [booking.id, file_url]
    );
